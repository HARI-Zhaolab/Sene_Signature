...
pdf(paste0("./outputfig/Fig4/2_survial_score.pdf"),width = 10, onefile = FALSE)
print(p)
dev.off()

# 保存一下高低分组样本信息，虽然后面分析按照的是中位数
save(ssdf,file = "./outputdata/Fig4/ssdf_tcga.Rdata")


# RFS画图20240115，这个是后续补充的两张图
### 生存
rm(list = ls())
options(stringsAsFactors = F)

load("./data/matedata.Rdata")
load("~/workplace/sene_score/outputdata/Fig1/all_clin_old.Rdata")
colnames(matedata)
colnames(all_clin)
matedata <- merge(matedata,all_clin,by.x = "name",by.y = "name")

data <- matedata[order(matedata[,"senescore"]),]
table(data$recrudescence)

data$surTime <- as.numeric(data$new_time)
data$vitalStat <- as.numeric(data$recrudescence)
data <- data[complete.cases(data[, "new_time"]), ]
